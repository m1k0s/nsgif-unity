...
*/		gif_data += 3;
		...
		*/
		...
		if ((gif->frame_image = gif->bitmap_callbacks.bitmap_create(gif->width, gif->height, gif->bitmap_callbacks.user_data)) == NULL) {
			gif_finalise(gif);
			return GIF_INSUFFICIENT_MEMORY;
		}
...
static gif_result gif_initialise_sprite(gif_animation *gif, unsigned int width, unsigned int height) {
	...
	*/
	assert(gif->bitmap_callbacks.bitmap_create);
	if ((buffer = gif->bitmap_callbacks.bitmap_create(max_width, max_height, gif->bitmap_callbacks.user_data)) == NULL)
		return GIF_INSUFFICIENT_MEMORY;
	assert(gif->bitmap_callbacks.bitmap_destroy);
	gif->bitmap_callbacks.bitmap_destroy(gif->frame_image, gif->bitmap_callbacks.user_data);
...
gif_result gif_decode_frame(gif_animation *gif, unsigned int frame) {
	...
	*/
	...
	frame_data = (void *)gif->bitmap_callbacks.bitmap_get_buffer(gif->frame_image, gif->bitmap_callbacks.user_data);
	if (!frame_data)
		return GIF_INSUFFICIENT_MEMORY;
	...
	*/
	if (gif->frames[frame].virgin) {
		if (gif->bitmap_callbacks.bitmap_test_opaque)
			gif->frames[frame].opaque = gif->bitmap_callbacks.bitmap_test_opaque(gif->frame_image, gif->bitmap_callbacks.user_data);
		else
			gif->frames[frame].opaque = false;
		gif->frames[frame].virgin = false;
	}
	if (gif->bitmap_callbacks.bitmap_set_opaque)
		gif->bitmap_callbacks.bitmap_set_opaque(gif->frame_image, gif->frames[frame].opaque, gif->bitmap_callbacks.user_data);
	if (gif->bitmap_callbacks.bitmap_modified)
		gif->bitmap_callbacks.bitmap_modified(gif->frame_image, gif->bitmap_callbacks.user_data);
